...
export default function HomePage() {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      alert(err.response?.data?.message || err.message || 'Error deleting task');
    }
  };
      ...
      <ul>
        {tasks.map(task => (
          <li key={task.id} style={{ marginBottom: '10px' }}>
            ...
            <strong>Created At:</strong> {new Date(task.createdAt).toLocaleString()} <br />
            <button 
              style={{ marginTop: 5, color: 'white', backgroundColor: 'red', border: 'none', padding: '5px 10px', cursor: 'pointer' }} 
